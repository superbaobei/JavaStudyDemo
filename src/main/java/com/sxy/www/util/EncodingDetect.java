...
public class EncodingDetect {
    ...
    public static String getJavaEncode(MultipartFile file) throws IOException {
        BytesEncodingDetect s = new BytesEncodingDetect();
        String fileCode = BytesEncodingDetect.javaname[s.detectEncoding(file)];
        return fileCode;
    }
...
class BytesEncodingDetect extends Encoding {
    ...
    /**
     * 重载上面的方法,兼容MultipartFile
     * @param file
     * @return
     * @throws IOException
     */
    public int detectEncoding(MultipartFile file) throws IOException {
        byte[] rawtext = getFileBytes(file);
        return detectEncoding(rawtext);
    }
